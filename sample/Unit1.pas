...
uses
  ...
  Dialogs, StdCtrls, AbrahaoUtil, Controls, IniFiles;
...
begin
  ...
  TOKEN := iniFile.ReadString('CONFIG', 'token', 'ABRAHAO-TOKEN');
  CUSTOMURL := iniFile.ReadString('CONFIG', 'customURL', '');
  iniFile.Free;

  edtToken.Text := TOKEN;
  edtURL.Text := CUSTOMURL;

  apiParameter := TRequestParam.create;
  apiParameter.token := Pchar(TOKEN);
  apiParameter.customUrl := PChar(CUSTOMURL);
end;
...
begin
  TOKEN := edtToken.Text;
  apiParameter.token := Pchar(TOKEN);
  CUSTOMURL := edtUrl.Text;
  apiParameter.customUrl := Pchar(CUSTOMURL);

  iniFile := TIniFile.Create(ExtractFileDir(Application.ExeName)+'\config.ini');
  iniFile.WriteString('CONFIG', 'token', TOKEN);
  iniFile.WriteString('CONFIG', 'customURL', CUSTOMURL);
  iniFile.Free;
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := deleteProduct(apiParameter, PChar(edtParametro.Text));
...
begin
  ...
  begin
    ...
    memo1.Lines.Add('Registros: ' + IntToStr(orderResult.data.Count));
    ...
    begin
      ...
      begin
        ...
        memo1.Lines.Add('  ');
        for z := 0 to orderItem.options.Count - 1 do
        begin
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := setOrderAsReceived(apiParameter, edtParametro.Text);
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := closeTable(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := cancelTable(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := closeCard(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := cancelCard(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := deleteTable(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := deleteCard(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := deleteUser(apiParameter, StrToInt(edtParametro.Text));
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := setEventAsReceived(apiParameter, edtParametro.Text);
...
var
  simpleResult : TSimpleResult;
begin
  simpleResult := reopenTable(apiParameter, StrToInt(edtParametro.Text));
